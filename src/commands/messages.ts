...
export function registerMessagesCommands(program: Command): void {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      const response = await withSpinner('Fetching messages...', () =>
        client.get(`/api/v1/e/${event}/p/${partnership}/messages`, { params: { q, page: opts.page, per_page: opts.perPage } })
      )
  ...
  cmd
    ...
    .action(async (id, _opts, cmd) => {
      ...
      const { event, partnership } = requireEventAndPartnership(g)
      const client = createClient({ test: g.test })
      const response = await withSpinner('Fetching message...', () =>
        client.get(`/api/v1/e/${event}/p/${partnership}/messages/${id}`)
      )
