...
export function registerAuthorizationsCommands(program: Command): void {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      const response = await withSpinner('Fetching authorizations...', () =>
        client.get(`/api/v1/e/${event}/p/${partnership}/authorizations`, { params: { q, page: opts.page, per_page: opts.perPage } })
      )
  ...
  cmd
    ...
    .action(async (id, _opts, cmd) => {
      ...
      const { event, partnership } = requireEventAndPartnership(g)
      const client = createClient({ test: g.test })
      const response = await withSpinner('Fetching authorization...', () =>
        client.get(`/api/v1/e/${event}/p/${partnership}/authorizations/${id}`)
      )
  ...
  cmd
    ...
    .action(async (id, opts, cmd) => {
      ...
      const response = await withSpinner('Updating authorization...', () =>
        client.patch(`/api/v1/e/${event}/p/${partnership}/authorizations/${id}`, { authorization: body })
      )
      printObject(response.data, { json: g.json })
    })
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      const response = await withSpinner('Updating authorizations...', () =>
        client.patch(`/api/v1/e/${event}/p/${partnership}/authorizations/bulk_update`, { authorizations })
      )
      if (g.json) {
